pub mod gpx_parser {
    ...
    impl Parser {
        pub fn new(name: &OsString) -> Option<Parser> {
            let f: File = match File::open(name) {
                Ok(file) => {
                    file
                },
                Err(_) => return None,
        ...
        #[unsafe(no_mangle)]
        pub extern fn Parser(name: &OsString) -> Option<Parser> {
            Parser::new(&name)
        }
        
        #[unsafe(no_mangle)]
        pub extern fn open_parser(self: &mut Self) -> Option<Track> {
            let route = self.process_gpx();
        ...
        #[unsafe(no_mangle)]
        pub extern fn process_gpx(self: &mut Self) -> Option<Vec<TrackRoute>> {
